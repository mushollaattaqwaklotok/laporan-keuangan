# app.py (FINAL) - Upload bukti ke GitHub (folder data/bukti/), tampil barang di publik, user/password panitia disertakan
...
# Multi-password untuk panitia (tetap - sesuai input user)
PANITIA_USERS = {
    "Ketua": "kelas3ku",
...
os.makedirs("data/bukti", exist_ok=True)
os.makedirs("data/penerimaan", exist_ok=True)
os.makedirs("data/bukti_barang", exist_ok=True)
...
def upload_file_to_repo(folder_path, filename, file_bytes):
    ...
    if GITHUB_TOKEN and GITHUB_REPO:
        ...
        with open(local_path, "wb") as f:
            f.write(file_bytes)
        # return local file path (relative)
        return local_path
...
if mode == PUBLIK_MODE:
    ...
    if df.empty:
        ...
        # ======================================================
        #  TAMPILAN BARANG DI PUBLIK (DITAMBAHKAN)
        # ======================================================
        ...
        if dfb.empty:
            ...
            # tampilkan tanpa kolom Bukti agar rapi
...
# ======================================================
    ...
    # -------------------------
    # DOWNLOAD CSV (tetap) - include barang download separately
    # -------------------------
    ...
    # -------------------------
    # LOG AKTIVITAS (tetap di bagian bawah)
    # -------------------------
